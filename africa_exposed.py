...
from collectors.crtsh_collector import collect_country_domains, collect_domain_domains_detailed, save_crtsh_data
...
def analyze_country(country_code, do_dns=False, do_subdomains=False):
    ...
    if do_dns:
        ...
        dns_data = collect_domains(crt_data, delay=0.3)
        save_dns_data(dns_data, country_code)

    if do_subdomains:
        print("\n🔎 Enumerating common subdomains")
        subdomain_data = enumerate_domains(crt_data, delay=0.3)
        save_subdomain_data(subdomain_data, country_code)

    discovered_ips = set()
    for domain_item in dns_data:
        for address in domain_item.get("records", {}).get("A", []) + domain_item.get("records", {}).get("AAAA", []):
            discovered_ips.add(address)
    for domain_item in subdomain_data:
        for record in domain_item.get("records", []):
            discovered_ips.add(record.get("ip"))
    discovered_ips.discard(None)
    internetdb_data = enrich_ips(discovered_ips, delay=0.5) if discovered_ips else []
    if internetdb_data:
...
def analyze_domain(domain, do_dns=True, do_subdomains=True):
    ...
    crt_info = collect_domain_domains_detailed(domain)
    crt_data = crt_info["domains"]
    print(f"   Certificate query status: {crt_info['certificate_query_status']} ({len(crt_data)} names)")

    dns_targets = [domain] + [name for name in crt_data if name != domain]
    dns_targets = dns_targets[:MAX_DNS_DOMAINS_PER_TARGET]
    dns_data = collect_domains(dns_targets, delay=0.2) if do_dns else []
    subdomain_data = enumerate_domains([domain], delay=0.2) if do_subdomains else []
    ...
    report = make_report(domain, crt_data, dns_data, subdomain_data, internetdb_data, crt_metadata=crt_info)
...
def run_study(manifest_path, salt="beacon-study", resume=False):
    """Run the reproducible passive measurement for every row in a study manifest."""
    ...
    for index, row in enumerate(rows, start=1):
        ...
        if resume and report_path.exists():
            ...
            try:
                ...
                pass
        print(f"\n[{index}/{len(rows)}] {row['country']} / {row['sector']} / {row['organization_id']} ({domain})")
        try:
            results.append(analyze_domain(domain, do_dns=True, do_subdomains=True))
...
Examples:
    ...
    python africa_exposed.py --study-manifest research/pilot_manifest.csv --resume
        """
    )
    ...
    if args.all:
        ...
        countries = [c for c in top_countries if c in AFRICAN_COUNTRIES]
    elif args.country:
    ...
    results = []
    for code_item in countries:
        result = analyze_country(code_item, do_dns=args.dns or args.collect, do_subdomains=args.subdomains or args.collect)
